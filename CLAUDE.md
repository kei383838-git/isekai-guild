...
`.png` / `.tres` などのアセットを削除する前に **必ず参照を確認** する：

1. `Grep` で全プロジェクトを走査して参照を見つける
   - 例：`Grep pattern: "forest_wall_tileset_v1"`（ファイル名の安定部分）
   - uid 参照もある場合：その uid 文字列でも grep
2. 参照が **0 件** であることを確認してから削除
3. 参照がある場合は、依存側 `.tres` / `.tscn` を **先に修正** する
   （差し替え or 該当行削除）。同じコミットで両方処理する

実例：2026-05-05、`forest_wall_tileset_v1_64.png` を「ダンジョンで不要」とだけ判断して削除したところ、村側の `terrain.tres` が同ファイルを参照しており Village ロード失敗を引き起こした。事前 grep で防げた。

アセット配置の方針は `docs/system/asset_structure.md` 参照。
